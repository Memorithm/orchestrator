...
data = main_path.read_text()

marker = '''fn validate_workspace(
    config: &RunConfig,
    workspace: &Path,
    item: &WorkItem,
...
'''
helper = '''fn run_portable_validation_plan(
    config: &RunConfig,
    workspace: &Path,
    item: &WorkItem,
    snapshot: &policy::PolicySnapshot,
...
if data.count(marker) != 1:
    raise SystemExit(f'validate_workspace marker count={data.count(marker)}')
...
'''
if data.count(old_loop) != 1:
    raise SystemExit(f'portable loop count={data.count(old_loop)}')
...
'''
if data.count(test_marker) != 1:
    raise SystemExit(f'main test marker count={data.count(test_marker)}')
data = data.replace(test_marker, tests + test_marker, 1)
main_path.write_text(data)
...
'''
if policy.count(policy_marker) != 1:
    raise SystemExit(f'policy test marker count={policy.count(policy_marker)}')
policy = policy.replace(policy_marker, policy_test + policy_marker, 1)
